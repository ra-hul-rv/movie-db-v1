...
export class WatchListComponent implements OnInit {
  ...
  getMovies() {
    ...
    this.movieService.getMovies(2010).subscribe(
      (data: any) => {
        if (!data) {
          this.allMovies = movieList.filter((movie: any) => movie.isWatched === true);
        } else {
          this.allMovies = data.filter((movie: any) => movie.isWatched === true);
        }
        this.totalMovies = this.allMovies.length;
        this.movies = this.allMovies.slice(0, this.pageSize);
      },
      (error: any) => {
        this.movies = movieList.filter((movie: any) => movie.isWatched === true);
        this.allMovies = movieList.filter((movie: any) => movie.isWatched === true);
        this.totalMovies = this.allMovies.length;
        this.movies = this.allMovies.slice(0, this.pageSize);
